...
def decrypt_password(key,encrypted_password):
    ...
    """
    f = Fernet(key)
    decrypted_password= f.decrypt(encrypted_password).decode() # Decode from bytes to string
...
def notify_feishu_bot(message):
    ...
    if response.status_code != 200:
        logger.info(f"Failed to send alert to Feishu status code: {response.status_code}")
    else:
